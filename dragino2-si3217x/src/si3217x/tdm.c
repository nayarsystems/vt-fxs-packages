...
static int init_slic_mbox(struct spi_device *spidev)
{
	...
	if (dma_set_coherent_mask(&spidev->dev, DMA_BIT_MASK(32)))
	{
		printk(KERN_INFO " dma_set_coherent_mask error");
		return -EIO;
	}
	...
	{
		printk(KERN_ERR PFX "ERR tdmdata.rx");
		return -ENOMEM;
	}
	...
	{
		printk(KERN_ERR PFX "ERR tdmdata.rx1");
		return -ENOMEM;
	}
	...
	{
		printk(KERN_ERR PFX "ERR tdmdata.tx");
		return -ENOMEM;
	}
	...
	{
		printk(KERN_ERR PFX "ERR tdmdata.tx1");
		return -ENOMEM;
	}
...
#define INTERRUPT_TXRX_HANDLER(DIRECTION, direction, action)                                                                              \
	...
	{                                                                                                                                     \
		...
		/*temp = AR9331_REG_READ(AR9331_SLIC_MBOX_INT_STATUS);*/                                                                          \
                                                                                                                                          \
		if (!tdmdata.direction##_descr0->OWN && tdmdata.direction##_descr1->OWN)                                                          \
		{ /* CPU owns descriptor 0 */                                                                                                     \
			if (wc.initialized)                                                                                                           \
			{                                                                                                                             \
		...
		else if (tdmdata.direction##_descr0->OWN && !tdmdata.direction##_descr1->OWN)                                                     \
		{ /* CPU owns descriptor 1 */                                                                                                     \
			if (wc.initialized)                                                                                                           \
			{                                                                                                                             \
...
static const struct proc_ops tdm_proc_ops = {
	.proc_open = tdm_proc_open,
	.proc_read = seq_read,
...
int si3217x_tdm_init(struct spi_device *spidev)
{
	...
	tdmdata.proc_entry = proc_create(DRV_NAME, 0400, NULL, &tdm_proc_ops);
